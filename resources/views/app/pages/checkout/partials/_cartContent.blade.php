@foreach (Cart::content() as $cartItem)
    ...
    {{--                                        @endif--}}
    @if ($cartItem->associatedModel == \Resto2web\Core\Domain\Meals\Models\Meal::class)
        <div class="mb-2">
                ...
                @if (count($cartItem->options))
                    <div class="align-self-center">
                        ...
                        @foreach ($cartItem->options as $key => $option)
                            @if (is_array($option))
                                @php($displayOptionGroup = \Resto2web\Core\Domain\Meals\Models\MealOptionGroup::findOrFail($key))
                                @foreach ($option as $optionId => $value)
                                    @if ($value)
                                        @php($displayOption = \Resto2web\Core\Domain\Meals\Models\MealOption::findOrFail($optionId))
                                        + {{ $displayOption->name }} <br>
                                    @endif
                                @endforeach
                            @else
                                @php($displayOption = \Resto2web\Core\Domain\Meals\Models\MealOption::findOrFail($option))
